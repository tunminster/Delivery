...
public class MeatOptionCreationCommandHandler : ICommandHandler<MeatOptionCreationCommand, MeatOptionCreationStatusContract>
{
    ...
    public async Task<MeatOptionCreationStatusContract> HandleAsync(MeatOptionCreationCommand command)
    {
        ...
        var product =
            await databaseContext.Products.SingleAsync(x => x.ExternalId == command.MeatOptionCreationContract.ProductId);
