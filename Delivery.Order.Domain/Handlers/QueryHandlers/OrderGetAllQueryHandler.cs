...
{
    public record OrderGetAllQuery(string StoreId, int PageSize, int PageNumber, string FreeTextSearch) : IQuery<OrderPagedContract>;
    ...
    public class OrderGetAllQueryHandler : IQueryHandler<OrderGetAllQuery, OrderPagedContract>
    {
        ...
        public async Task<OrderPagedContract> Handle(OrderGetAllQuery query)
        {
            ...
            var orderCacheKey = $"Database-{executingRequestContextAdapter.GetShard().Key}-{query.PageSize}-{query.PageNumber}-{storeId}-{nameof(OrderGetAllQueryHandler).ToLowerInvariant()}-{query.FreeTextSearch}";    
